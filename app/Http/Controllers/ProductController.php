...
class ProductController extends Controller
{
    public static function makeProductArray()
    {
        ...
        // return $products;
        return view('/index', ['cds' => $cds, 'books' => $books]);
    }
            ...
            if ($product['id'] == $id) {
                switch ($product['type']) {
                    case "cd":
                        ...
                        $products[] = $cdproduct;
                        return view('myCd', ['cdproduct' => $cdproduct]);
                        break;
                    case "book":
                        $bookproduct = new BookProduct(
                            $product['id'],
                        ...
                        return view('myBook', ['bookproduct' => $bookproduct]);
                        break;
                }
                break;
            }
    ...
    public static function addNewProduct()
    {
        ...
        if ($producttype == 'cd') $newProduct['playlength'] = request('playpg');
        if ($producttype == 'book') $newProduct['numpages'] = request('playpg');
    ...
    public static function updateProductWithId(int $id)
    {
        ...
        foreach ($productsJson as $product) {
            if ($product['id'] == $id) {
                ...
                if ($product['type'] == 'cd') $product['playlength'] = request('playpg');
                if ($product['type'] == 'book') $product['numpages'] = request('playpg');
            }
            $products[] = $product;
        }
